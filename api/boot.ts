...
async function runStartupMigrations() {
  try {
    ...
    // +₹1/km for all ≤7 seat cars — one-time, idempotent via siteSettings flag
    try {
      const [flagRows] = await db.execute(sql.raw(
        `SELECT value FROM siteSettings WHERE \`key\` = 'priceKmInc_20260703' LIMIT 1`
      )) as any;
      const alreadyDone = Array.isArray(flagRows) && flagRows.length > 0;
      if (!alreadyDone) {
        await db.execute(sql.raw(`UPDATE cars SET pricePerKm = pricePerKm + 1 WHERE seats <= 7 AND isAvailable = TRUE`));
        await db.execute(sql.raw(`INSERT INTO siteSettings (\`key\`, value) VALUES ('priceKmInc_20260703', '1') ON DUPLICATE KEY UPDATE value = '1'`));
        console.log('[startup] Price increase applied: +₹1/km for ≤7 seat cars');
      }
    } catch (e) { console.error('[startup] Price increase error:', e); }

    // Force-update subtitle fares for all Delhi routes (+₹1/km rates)
    try {
      await db.execute(sql.raw(`
        UPDATE linkHubCards SET subtitle = CASE linkUrl
          WHEN '/cab/delhi-to-kedarnath'    THEN '470 km · 10-11 hrs · from ₹7,890'
          WHEN '/cab/delhi-to-manali'       THEN '540 km · 12-14 hrs · from ₹9,030'
          WHEN '/cab/delhi-to-shimla'       THEN '350 km · 7-8 hrs · from ₹5,940'
          WHEN '/cab/delhi-to-rishikesh'    THEN '250 km · 5-6 hrs · from ₹4,310'
          WHEN '/cab/delhi-to-haridwar'     THEN '220 km · 4-5 hrs · from ₹3,830'
          WHEN '/cab/delhi-to-chandigarh'   THEN '260 km · 4-5 hrs · from ₹4,480'
          WHEN '/cab/delhi-to-jaipur'       THEN '280 km · 4-5 hrs · from ₹4,800'
          WHEN '/cab/delhi-to-agra'         THEN '230 km · 3-4 hrs · from ₹3,990'
          WHEN '/cab/delhi-to-dharamshala'  THEN '475 km · 10-11 hrs · from ₹7,970'
          WHEN '/cab/delhi-to-nainital'     THEN '310 km · 6-7 hrs · from ₹5,290'
          WHEN '/cab/delhi-to-dehradun'     THEN '300 km · 5-6 hrs · from ₹5,130'
          WHEN '/cab/delhi-to-mussoorie'    THEN '310 km · 6-7 hrs · from ₹5,290'
          WHEN '/cab/delhi-to-mathura'      THEN '175 km · 2-3 hrs · from ₹3,090'
          WHEN '/cab/delhi-to-amritsar'     THEN '460 km · 7-8 hrs · from ₹7,730'
          WHEN '/cab/delhi-to-kashmir'      THEN '820 km · 14-16 hrs · from ₹13,580'
          WHEN '/cab/delhi-to-vaishno-devi' THEN '650 km · 12-13 hrs · from ₹10,810'
          WHEN '/cab/delhi-to-ludhiana'     THEN '310 km · 5-6 hrs · from ₹5,290'
          WHEN '/cab/delhi-to-ayodhya'      THEN '640 km · 10-12 hrs · from ₹10,650'
          WHEN '/cab/delhi-to-banaras'      THEN '820 km · 12-14 hrs · from ₹13,580'
          WHEN '/cab/delhi-to-jodhpur'      THEN '600 km · 9-10 hrs · from ₹10,000'
          WHEN '/cab/delhi-to-udaipur'      THEN '665 km · 10-11 hrs · from ₹11,060'
          WHEN '/cab/delhi-to-pushkar'      THEN '395 km · 6-7 hrs · from ₹6,670'
          WHEN '/cab/delhi-to-corbett'      THEN '250 km · 5-6 hrs · from ₹4,310'
          WHEN '/cab/delhi-to-kasauli'      THEN '315 km · 5-6 hrs · from ₹5,370'
          WHEN '/cab/delhi-to-dalhousie'    THEN '555 km · 10-11 hrs · from ₹9,270'
          WHEN '/cab/delhi-to-lucknow'      THEN '555 km · 7-8 hrs · from ₹9,270'
          WHEN '/cab/delhi-to-prayagraj'    THEN '645 km · 9-10 hrs · from ₹10,730'
          WHEN '/cab/delhi-to-vrindavan'    THEN '155 km · 2.5-3 hrs · from ₹2,770'
          WHEN '/cab/delhi-to-spiti'        THEN '785 km · 14-16 hrs · from ₹13,010'
          WHEN '/cab/delhi-to-mount-abu'    THEN '780 km · 12-13 hrs · from ₹12,930'
          WHEN '/cab/delhi-to-lansdowne'    THEN '265 km · 5-6 hrs · from ₹4,560'
          ELSE subtitle END
